...
public:
  ...
  void SetDefaultKeyId(const std::string& keyId) override;
  void AddKeyId(const std::string& keyId) override;
    ...
    drained_(true)
{
  ...
  if (session_.empty())
  {
    Log(SSD_HOST::LL_ERROR, "Cannot perform License update, no session available");
    return;
  }
...
bool WV_CencSingleSampleDecrypter::OpenVideoDecoder(const SSD_VIDEOINITDATA *initData)
{
  ...
  switch (initData->codec)
  {
    ...
    default:
      ...
      Log(SSD_HOST::LL_ERROR, "WVDecoder: Unknown codec %i", initData->codec);
      break;
  }
  ...
  switch (initData->codecProfile)
  {
    ...
    case SSD::CodecProfile::CodecProfileNotNeeded:
      ...
      break;
    default:
      Log(SSD_HOST::LL_ERROR, "WVDecoder: Unknown codec profile %i", initData->codecProfile);
  ...
  switch (initData->videoFormats[0])
  {
    ...
    case SSD::SSD_VIDEOFORMAT::VideoFormatI420:
      ...
      break;
    default:
      Log(SSD_HOST::LL_ERROR, "WVDecoder: Unknown video format %i", initData->videoFormats[0]);
...
void WV_CencSingleSampleDecrypter::SetDefaultKeyId(const std::string& keyId)
{
  memcpy(defaultKeyId_, keyId.c_str(), 16);
}

void WV_CencSingleSampleDecrypter::AddKeyId(const std::string& keyId)
{
  WVSKEY key;
  key.keyid = keyId;
