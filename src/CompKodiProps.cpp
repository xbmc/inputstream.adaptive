  ...
  for (const auto& prop : props)
  {
    ...
    else if (prop.first == PROP_DRM && !prop.second.empty())
    {
      if (!ParseDrm(prop.second))
        LOG::LogF(LOGERROR, "Cannot parse \"%s\" property, wrong or malformed data.", prop.first.c_str());
...
bool ADP::KODI_PROPS::CCompKodiProps::ParseDrm(std::string data)
{
  ...
  for (auto& jChildObj : jDoc.GetObject())
  {
    ...
    {
      LOG::LogF(LOGERROR,
                "Cannot parse key system \"%s\" value on DRM property, wrong data type",
                keySystem);
      continue;
    }
    ...
    LogDrmJsonDictKeys(jDictVal, keySystem); // todo: <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< comment this, maybe an appropriate config log elsewere
  }
  ...
  for (auto& jChildObj : jDoc.GetObject())
  {
    ...
    if (jDictVal.HasMember("wrapper_params") && jDictVal["wrapper_params"].IsObject())
    {
      ...
      for (auto& cWrapParam : jDictVal["wrapper_params"].GetObject())
      {
        ...
        {
          drmCfg.m_license.m_wrapperParams.emplace(cWrapParam.name.GetString(), cWrapParam.value.GetString());
        }
      }
    }
