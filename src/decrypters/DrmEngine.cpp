...
bool GetCapabilities(std::optional<bool> isForceSecureDecoder,
                     std::optional<bool> drmCfgIsSecureDecoderEnabled,
                     ...
                     DRMSession& session)
{
  ...
  session.drm->GetCapabilities(session.decrypter, defaultKid, dMedia, session.capabilities);

  if (session.capabilities.flags & DRM::DecrypterCapabilites::SSD_INVALID)
  {
    return false;
  }
  else if (session.capabilities.flags & DRM::DecrypterCapabilites::SSD_SECURE_PATH)
  {
    ...
    if (disableSecureDecoder)
    {
      ...
      session.capabilities.flags &= ~DRM::DecrypterCapabilites::SSD_SECURE_DECODER;
    }
  }
                                        ...
                                        bool canCleanupSessions)
{
  ...
  for (DRMSession& s : m_sessions)
  {
    ...
    // share same session when: init data (KID) is the same, otherwise check if there is license key by media type
    else if ((!s.initData.empty() && s.initData == drmInfo.initData) ||
  ...
  if (!session)
  {
    ...
    DRMSession newSes;
    newSes.id = "session_" + std::to_string(m_sessionsCounter++);
    newSes.drm = m_drms[m_keySystem];
    newSes.mediaType = mediaType;
    newSes.initData = drmInfo.initData;
    ...
    newSes.drmId = newSes.decrypter->GetSessionId();
    ...
    LOG::Log(LOGDEBUG, "Initialized new DRM session (ID: %s, KID: %s, INTERNAL ID: %s)",
             session->drmId.c_str(), drmInfo.defaultKid.c_str(), session->id.c_str());
  }
  else
  {
    DRMSession newSes;
    // Make a copy of the DRMSession for two reasons:
    // 1) Shared session uses (to take in account DeleteSessionsByType)
    // 2) DRM capabilities can be different between streams (e.g. audio/video)
    newSes.id = "session_" + std::to_string(m_sessionsCounter++);
    newSes.drmId = session->drmId;
    newSes.drm = session->drm;
    newSes.decrypter = session->decrypter;
    newSes.mediaType = mediaType;
    newSes.initData = drmInfo.initData;

    if (mediaType != session->mediaType || drmInfo.initData != session->initData)
    {
      if (!GetCapabilities(isForceSecureDecoder, drmPropCfg.isSecureDecoderEnabled, drmInfoKidBytes,
                           newSes))
      {
    ...
    m_sessions.emplace_back(newSes);
    session = &m_sessions.back();
    LOG::Log(LOGDEBUG, "Reused existing DRM session (ID: %s, KID: %s, INTERNAL ID: %s)",
             session->drmId.c_str(), drmInfo.defaultKid.c_str(), session->id.c_str());
  }
  ...
  if (session->capabilities.flags & DRM::DecrypterCapabilites::SSD_SECURE_PATH)
    cryptoSession.SetKeySystem(KSToCryptoKeySystem(m_keySystem));

  if (session->capabilities.flags & DRM::DecrypterCapabilites::SSD_SECURE_PATH &&
      session->capabilities.flags & DRM::DecrypterCapabilites::SSD_SUPPORTS_DECODING)
  {
    LOG::Log(LOGDEBUG, "Secure crypto session enabled to DRM session (ID: %s, INTERNAL ID: %s)",
             session->drmId.c_str(), session->id.c_str());
      ...
      session->capabilities.flags & DRM::DecrypterCapabilites::SSD_SECURE_DECODER)
  {
    ...
    LOG::Log(LOGDEBUG, "Secure crypto decoder enabled to DRM session (ID: %s, INTERNAL ID: %s)",
             session->drmId.c_str(), session->id.c_str());
